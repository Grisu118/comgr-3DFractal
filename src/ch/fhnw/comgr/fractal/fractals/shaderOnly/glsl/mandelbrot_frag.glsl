...
void main() {
    ...
    vec2 position = gl_FragCoord.xy; // gets the location of the current pixel in the intervals [0..1] [0..1]
    vec3 color = vec3(0.0,0.0,0.0); // initialize color to black

    vec2 z = vec2(position.x / outputSize.x, position.y / outputSize.y); // z.x is the real component z.y is the imaginary component
